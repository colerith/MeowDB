...
export const DEFAULT_EVENTS_PROMPT = [
  ...
  '- id: 事件ID（可留空）',
  '- messageIndex: 事件来源消息楼层数（整数，表示“根据消息楼层数生成”）',
  '- time: 事件时间文本',
  '- location: 事件地点文本',
  '- summary: 事件摘要（客观事实记录故事骨架，保留核心细节，150-300字，单行不换行）',
  '- tag: 事件标记，仅允许 日常/转折/关键/大事件',
  '- pinned: 是否置顶（boolean）',
  '',
  '规则：',
  '- 最新事件必须放在最上方，messageIndex 必须可追溯',
  '- 常规事件默认 tag=日常，重大推进可标记 转折/关键/大事件',
  '- 置顶事件最多3条，优先保留仍在持续影响剧情的事件',
  '- 若存在置顶事件，后续关系/承诺/待办更新必须优先参考这些置顶事件',
  '- 状态卡字段 time/scene/plot 以最新事件自动映射为准，不要维护两份互相冲突的内容',
].join('\n');
...
export function buildPrompt(
  ...
  const user = [
    ...
    '【要求】',
    '- serial 在当前基础上递增',
    '- time/scene/plot 会由最新事件自动映射，避免重复维护',
    '- 根据剧情更新 relations / events / echoes / todos / enigmas / seeds（scene/plot由事件同步）',
    '- 优先保证 relations_json 字段完整、可直接用于前端卡片展示',
    '- events_json 维护时间轴（最新在前），summary 必须为150-300字单行客观记录',
