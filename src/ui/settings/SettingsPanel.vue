        ...
        <div v-if="activeTab === 'settings'" key="settings" class="meowdb-tab-content active">
          ...
          <section class="meowdb-settings-section">
            ...
            <div class="meowdb-profile-toolbar">
              ...
              <label class="meowdb-setting-stack meowdb-profile-name">
                <span>方案名称</span>
                <input
                  class="meowdb-input"
                  v-model="activeProfileName"
                  type="text"
                  placeholder="例如：OpenRouter-省钱版"
                />
              </label>
            </div>

            <div class="meowdb-setting-actions">
              <button class="menu_button meowdb-tool-btn" type="button" @click="saveAsNewProfile">??????</button>
              <button class="menu_button meowdb-tool-btn" type="button" @click="updateCurrentProfile">??????</button>
              <button class="menu_button meowdb-tool-btn" type="button" @click="copyCurrentProfile">??????</button>
              <button class="menu_button meowdb-tool-btn" type="button" @click="exportCurrentProfile">??????</button>
              <button class="menu_button meowdb-tool-btn" type="button" @click="triggerImportProfile">????</button>
              <button
...
const activeProfileName = computed({
  get: () => activeProfile.value?.name || '',
  set: value => {
    if (!activeProfile.value) return;
    activeProfile.value.name = value.trim() || '未命名方案';
  },
});
...
function ensureProfiles() {
  if (!Array.isArray(settings.value.api_profiles)) {
    settings.value.api_profiles = [];
  }

  if (settings.value.api_profiles.length === 0) {
    settings.value.api_profiles.push(createProfile('默认配置'));
  }

  const activeExists = settings.value.api_profiles.some(item => item.id === settings.value.api_active_profile_id);
  if (!activeExists) {
...
async function copyCurrentProfile() {
  ...
  const payload = {
    ...
    enabled: activeProfile.value.enabled,
    url: activeProfile.value.url,
    key: activeProfile.value.key,
    model: activeProfile.value.model,
    temperature: activeProfile.value.temperature,
    max_tokens: activeProfile.value.max_tokens,
  };
...
async function onImportProfileFile(event: Event) {
  ...
  try {
    ...
    const profile: ApiProfile = {
      id: makeProfileId(),
      name: (data.name || file.name.replace(/\.json$/i, '') || '????').trim(),
...
async function fetchModels() {
  ...
  try {
    ...
    if (!settings.value.api_model) {
      settings.value.api_model = ids[0];
    }
    syncFieldsToActiveProfile();
